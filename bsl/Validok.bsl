...
Функция Валидация(Знач Шаблон, Знач Данные, ТекущееИмя = "") Экспорт
	...
	Для каждого КЗ Из Шаблон Цикл
		...
		Если НЕ ЗначениеЗаполнено(Значение) Тогда
			...
			Если Обязательность Тогда
				СтруктураВозврата.Вставить(Имя,
					ПодставитьПараметрыВСтроку(ЛокализованнаяСтрока("ВалидацииЗначениеДолжноБытьЗадано"),
						?(ПустаяСтрока(ТекущееИмя), "", ТекущееИмя + ".") + Имя));
			КонецЕсли;
			Продолжить;
		...
		Если ТипЗнч(Значение) <> ТипЗначения Тогда
			СтруктураВозврата.Вставить(Имя, ПодставитьПараметрыВСтроку(ЛокализованнаяСтрока("ВалидацииЗначениеДолжноБытьТипа"), ТипЗначения));
			Продолжить;
		КонецЕсли;
		...
		Если ТипЗначения = Тип("Соответствие") И Правила["Шаблон"] <> Неопределено Тогда
			...
			Если М.Количество() <> 0 Тогда
				Для каждого КЗ Из М Цикл
					СтруктураВозврата.Вставить(КЗ.Ключ,
						КЗ.Значение);
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
...
Функция СериализацияВJson(Объект, ПереносСтрокJSON = Неопределено) Экспорт
	...
	ЗаписьJson.УстановитьСтроку(Новый ПараметрыЗаписиJSON(?(ПереносСтрокJSON = Неопределено, ПереносСтрокJSON.Нет, ПереносСтрокJSON)));
