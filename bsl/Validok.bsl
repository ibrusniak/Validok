...
Функция Валидация(Знач Шаблон, Знач Данные, ИмяОбъектаВерхнегоУровня = "") Экспорт
	...
	СтруктураВозврата.Вставить("ВалидацияПройдера", Истина);
	СтруктураВозврата.Вставить("ОшибкиВалидации", Новый Соответствие);
	...
	Для каждого КЗ Из Шаблон Цикл
		...
		Если ТипЗначения = Тип("Соответствие") И Правила["Шаблон"] <> Неопределено Тогда
			...
			РВалидации = Валидация(Правила["Шаблон"], Значение, Имя);
			
			Если РВалидации.Количество() <> 0 Тогда
				
				СтруктураВозврата["ВалидацияПройдера"] = Ложь;
				
				Для каждого КЗ Из РВалидации["ОшибкиВалидации"] Цикл
					
					Если КЗ.Ключ = "ВалидацияПройдера" Тогда
						Продолжить;
					КонецЕсли;
