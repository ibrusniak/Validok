...
Функция ВалидацияЗначенияТипаСтрока(Значение, Шаблон)
	...
	Если Шаблон["РегулярноеВыражение"] <> Неопределено Тогда
		
		Попытка
			СоответствуетРВ = ПроверитьСтрокуНаСоответствиеРегулярномуВыражению(Значение, Шаблон["РегулярноеВыражение"])		
		Исключение
