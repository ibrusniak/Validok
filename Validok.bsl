...
Функция БазовыеПроверкиШаблона(ШаблонJson)
	...
	Если Шаблон["Тип"] = Неопределено Тогда
		ВызватьИсключение
			НСтр("en = 'Template must contain the required field ''Тип''';
			|ru = 'Шаблон должен содержать обязательное поле ''Тип''';
			|uk = 'Шаблон має містити обов''язкове поле ''Тип'''");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение
			НСтр("en = 'The ''Тип'' field in the template specifies a data type not supported by the 1C system';
			|ru = 'В поле шаблона ''Тип'' указан тип данных, не поддерживаемый системой 1С';
			|uk = 'У полі шаблону ''Тип'' вказано тип даних, який не підтримується системою 1С'");
	КонецПопытки;
	
	Если Не ТипЗначенияПоддерживаетсяВалидатором(ТипЗначенияВШаблоне) Тогда
		ВызватьИсключение
				ПодставитьПараметрыВСтроку(
					НСтр("en = 'Value type specified in the template (''%1'') is not supported by the validator';
					|ru = 'Тип значения, указанный в шаблоне (''%1''), не поддерживается валидатором';
					|uk = 'Тип значення, вказаний у шаблоні (''%1''), не підтримується валідатором'"), ТипЗначенияВШаблоне);
	КонецЕсли;
	
	Если Шаблон["Обязательно"] = Неопределено Тогда
		ВызватьИсключение
			НСтр("en = 'Template must contain the required field ''Обязательно''';
			|ru = 'Шаблон должен содержать обязательное поле ''Обязательно''';
			|uk = 'Шаблон має містити обов''язкове поле ''Обязательно'''");
	КонецЕсли;
	
	Если ТипЗнч(Шаблон["Обязательно"]) <> Тип("Булево") Тогда
		ВызватьИсключение
			ПодставитьПараметрыВСтроку(
				НСтр("en = 'Field ''Обязательно'' must be of type ''%1''';
				|ru = 'Поле ''Обязательно'' должно быть типа ''%1''';
				|uk = 'Поле ''Обязательно'' має бути типу ''%1'''"), Тип("Булево"));
	КонецЕсли;
	...
	Для каждого КЗ Из Шаблон Цикл
		...
		Если КЗ.Ключ = "Шаблон" Тогда
			...
			Иначе
				...
				ВызватьИсключение
					ПодставитьПараметрыВСтроку(
						НСтр("en = 'Field ''Шаблон'' must be of a composite type (%1)';
						|ru = 'Поле ''Шаблон'' должно быть составного типа (%1)';
						|uk = 'Поле ''Шаблон'' має бути складного типу (%1)'"), Тип("Соответствие"));
		...
		Иначе
			...
			Если РазрешенныеВШаблонеИменаКлючей().Найти(КЗ.Ключ) = Неопределено Тогда
				...
				ВызватьИсключение
					ПодставитьПараметрыВСтроку(
						НСтр("en = 'Unknown key in template: ''%1''';
						|ru = 'Неизвестный ключ в шаблоне: ''%1''';
						|uk = 'Невідомий ключ у шаблоні: ''%1'''"), КЗ.Ключ);
...
Функция ВалидацияЗначенияТипаЧисло(Значение, Шаблон)
	...
	ПроверитьГраничныеЗначения("Мин", "Макс", Мин, Макс);
...
Процедура ПроверитьГраничныеЗначения(ИмяПоляМинимальногоНачения,
									 ИмяПоляМаксимальногоЗначения,
									 Мин,
									 Макс)
	...
	Если Мин <> Неопределено И ТипЗнч(Мин) <> Тип("Число") Тогда
		...
		ВызватьИсключение
			ПодставитьПараметрыВСтроку(
				НСтр("en = 'Field ''%2'' must be of type ''%1''';
				|ru = 'Поле ''%2'' должно быть типа ''%1''';
				|uk = 'Поле ''%2'' має бути типу ''%1'''"), Тип("Число"), ИмяПоляМинимальногоНачения);
	...
	Если Макс <> Неопределено И ТипЗнч(Макс) <> Тип("Число") Тогда
		...
		ВызватьИсключение
			ПодставитьПараметрыВСтроку(
				НСтр("en = 'Field ''%2'' must be of type ''%1''';
				|ru = 'Поле ''%2'' должно быть типа ''%1''';
				|uk = 'Поле ''%2'' має бути типу ''%1'''"), Тип("Число"), ИмяПоляМаксимальногоЗначения);
	...
	Если Мин <> Неопределено И Макс <> Неопределено И
		...
		ВызватьИсключение
			НСтр("en = 'The lower bound of the value must be less than the upper bound';
			|ru = 'Нижняя граница значения должна быть меньше верхней';
			|uk = 'Нижня межа значення має бути меншою за верхню'");
