	...
	Если Не ТипЗначенияПоддерживаетсяВалидатором(ТипЗначенияВШаблоне) Тогда
		ВызватьИсключение
				ПодставитьПараметрыВСтроку(
					НСтр("en = 'Template error. Value type specified in the template (%1) is not supported by the validator. Supported types:
					|%2'; ru = 'Ошибка шаблона. Тип значения, указанный в шаблоне (%1), не поддерживается валидатором. Поддерживаемые валидатором типы: %2';
	...
	Если Мин <> Неопределено И ТипЗнч(Мин) <> Тип("Число") Тогда
		...
		ВызватьИсключение
			ПодставитьПараметрыВСтроку(
				...
				|uk = 'Помилка шаблону. Поле ''%2'' має бути типу ''%1'''"), Тип("Число"), "Мин");
	...
	Если Макс <> Неопределено И ТипЗнч(Макс) <> Тип("Число") Тогда
		...
		ВызватьИсключение
			ПодставитьПараметрыВСтроку(
				...
				|uk = 'Помилка шаблону. Поле ''%2'' має бути типу ''%1'''"), Тип("Число"), "Макс");
...
Процедура ПроверитьДопустимыеЗначения(Шаблон)
	...
	Если ДопустимыеЗначения <> Неопределено Тогда
		...
		Если ТипЗнч(ДопустимыеЗначения) <> Тип("Массив") Тогда
			ВызватьИсключение ТекстовкаИсключения;
		КонецЕсли;
...
Функция ВалидацияЗначенияТипаЧисло(Значение, Шаблон, Знач ИмяПоля, ИмяПоляВерхнегоУровня = Неопределено)
	...
	Если Мин <> Неопределено И Значение < Мин Тогда
		...
		СтруктураВозврата.Ошибки.Вставить(ИмяПоля,
	...
	Если ДопустимыеЗначения <> Неопределено Тогда
		...
		Если ДопустимыеЗначения.Найти(Значение) = Неопределено Тогда
			...
			ПодставитьПараметрыВСтроку(
				...
				|uk = 'Значення має входити до множини: %1'"), МассивСтрокой(ДопустимыеЗначения)));
			Возврат СтруктураВозврата;
...
Функция ВалидацияЗначенияТипаМассив(Массив, Шаблон, ИмяПоля, ИмяПоляВерхнегоУровня = Неопределено)
	...
	Если Мин <> Неопределено И Массив.Количество() < Мин Тогда
		...
		СтруктураВозврата.Результат = Ложь;
		СтруктураВозврата.Ошибки.Вставить(ИмяПоля,
			ПодставитьПараметрыВСтроку(НСтр("en = 'Array size must be greater than or equal to %1';
			|ru = 'Размер массива должен быть больше или равен %1';
			|uk = 'Розмір масиву повинен бути більшим або дорівнювати %1'"), Мин));
		Возврат СтруктураВозврата;
	...
	Если Макс <> Неопределено И Массив.Количество() > Макс Тогда
		...
		СтруктураВозврата.Результат = Ложь;
		СтруктураВозврата.Ошибки.Вставить(ИмяПоля,
			ПодставитьПараметрыВСтроку(НСтр("en = 'Array size must be less than or equal to %1';
			|ru = 'Размер массива должен быть меньше или равен %1';
			|uk = 'Розмір масиву повинен бути меньше або дорівнювати %1'"), Макс));
		Возврат СтруктураВозврата;
